...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::COC)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("COFFEECOIN")
    updateDisplayUnit();
}
...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeCOFFEECOINAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCOFFEECOINAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeCOFFEECOINAmount = strAnonymizeCOFFEECOINAmount.remove(strAnonymizeCOFFEECOINAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCOFFEECOINAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeCOFFEECOINAmount * COIN) nMaxToAnonymize = nAnonymizeCOFFEECOINAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeCOFFEECOINAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeCOFFEECOINAmount));
        strAnonymizeCOFFEECOINAmount = strAnonymizeCOFFEECOINAmount.remove(strAnonymizeCOFFEECOINAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCOFFEECOINAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend()
{
    ...
    if (!fEnableDarksend) {
        ...
        if (nAnonymizeCOFFEECOINAmount == 0) {
