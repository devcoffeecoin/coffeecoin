...
*Setup instructions for a gitian build of COFFEECOIN using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the COFFEECOIN
...
- [Building ALQO](#building-coffeecoin)
...
Clone the git repositories for coffeecoin and gitian and then checkout the coffeecoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the coffeecoin repository
...
gitian builds' in [doc/release-process.md](release-process.md) in the coffeecoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/coffeecoin/.git/
...
./bin/gbuild --commit coffeecoin=${COMMIT} --url coffeecoin=${URL} ../coffeecoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit coffeecoin=${COMMIT} --url coffeecoin=${URL} ../coffeecoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit coffeecoin=${COMMIT} --url coffeecoin=${URL} ../coffeecoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/coffeecoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/coffeecoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/coffeecoin-build.assert
```
...
[coffeecoin/gitian.sigs](https://github.com/coffeecoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
